...
const TimeSlotItem = ({ hour, minute, handleConfirmModal, timezoneName, daySelected }) => {
    ...
    // # working solution
    const dynamicTimeSlot = momentTZ(dateSelected).format('h:mm a'); // formats the date for dynamic slot rendering
    return (
        <React.Fragment>
            <Button onClick={() => handleConfirmModal(dynamicTimeSlot, formattedTimeSlot24hr, (dateSelected+timezoneOffset))} className="calendarPage__timeslots--btn">{dynamicTimeSlot}</Button>
        </React.Fragment>
    )
}
